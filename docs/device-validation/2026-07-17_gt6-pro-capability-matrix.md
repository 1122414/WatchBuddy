...
| 能力 | 当前证据 | 当前状态 | 硬验收 |
|---|---|---|---|
| 应用内宠物 | 现有主页有 176 × 176 CSS 占位角色和点击入口 | 待实现真实资源 | 466 × 466 无裁切、可点击、切页停止动画 |
| Codex v2 输入 | 当前 hatch-pet 契约为 8 × 11、1536 × 2288、`spriteVersionNumber: 2` | 契约已核对 | schema、像素、透明度、帧和摘要全部通过 |
| 旧版输入 | 本机 `chibi-skadi` 为 8 × 9、1536 × 1872 且缺少 v2 标记 | 仅作拒绝/迁移样本 | 不得静默当作 v2 |
| 手表资源格式 | 已有 `watch-pet.json` Schema、纯 JS 语义校验和安全边界；尚无小帧或转换报告 | 格式完成，转换待实现 | 可版本化、可校验、离线可读 |
| 默认宠物 | 尚无项目自有宠物资产 | 待使用 hatch-pet 制作 | v2 QA 通过、来源和授权完整 |
| 状态动画 | 当前仅用 CSS 颜色、旋转和透明度区分状态 | 待实现 | 状态映射、一次性动作、降级和生命周期测试通过 |
| 资源渲染 | Lite Wearable 本地图片和逐帧切换尚无本工程证据 | 待 SDK/预览/真机 | 目标帧率稳定、内存和功耗可接受 |
| 服务端目录 | 当前 API 没有宠物目录 | 待实现 | 仅返回审核后元数据和固定摘要 |
| codex-pets.net 导入 | 网站当前同时可能存在 v2 和旧资源 | 待实现受控导入 | 不让手表直连，不接受未知授权或任意 URL |
