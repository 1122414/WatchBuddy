...
互动、设备注册、状态同步、安全令牌存储和受控宠物安装均已迁移。宠物安装使用 Network Kit
分页下载、Crypto Architecture Kit SHA-256、Core File Kit 应用沙箱和 Preferences 单条选择指针；
只有整包校验完成后才切换，失败继续使用上一只完整宠物。快捷回复会先保存非敏感幂等发件箱，
再从手表直接提交；瞬时错误按 400/800/1600ms 最多重试三次，应用重启后仍可继续。安静模式、
最近三条记忆、单条删除和二次点按确认清空也已接入 ArkTS 圆屏页面。此前
`00303168` 是命令行把
`DEVECO_SDK_HOME` 错指向
...
ArkTS 宠物库现可恢复已选宠物、读取鉴权目录、分页下载 73 帧资源、校验规范 Base64/PNG
魔数/长度/SHA-256、写入临时目录、回读全量资源并原子改名。安装清单与每帧摘要会一并保存；
应用重启时重新验证全部资源，损坏或选择指针不一致时清除动态选择并回退到 HAP 内置芽芽。
当前 `ApiConfig.ets` 的 HTTPS 地址仍为空，所以这条链路只完成了编译和静态/服务端契约验证，
...
智能穿戴 ArkTS 客户端会校验消息协议版本、消息类型、状态、来源、动作数量/重复项和服务端时间
下的过期状态；圆屏消息页提供二到四个服务端动作，回复不经过手机。记忆页仅读取鉴权接口，
删除前锁定具体 ID，清空操作必须连续确认两次。上述链路已通过 ArkTS 编译和静态契约测试，
...
- `npm run doctor:watch` 已完整通过，`npm run build:watch` 已成功生成包含 73 个芽芽帧的智能穿戴
  unsigned HAP；
- 新 ArkTS 表端已接入 Network Kit 注册和角色状态同步，令牌使用 Asset Store Kit；快捷回复、
  安静模式、记忆控制与受控宠物安装均已迁移，公网 HTTPS 和真机联网仍未验证；
