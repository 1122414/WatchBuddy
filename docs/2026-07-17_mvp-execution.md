...
- [x] 确认本机 `chibi-skadi` 是缺少 `spriteVersionNumber` 的 8 × 9 旧资源；它不能作为公开 v2
  默认宠物，但可在禁止再分发的前提下转换为 57 帧私人本地 HAP 输入；
...
- Lite Wearable 表端源码保留 `@system.fetch`、公网 WatchBuddy API 和 DeepSeek 互动；0.4.1
  未显式声明网络权限时安装成功，但真机显示无法连接服务。0.4.2 在不伪装 AI 输出的前提下，
  为聊聊、鼓励和晚安补充本地文字回应，核心宠物互动继续离线可用；
...
- 本机已有的 `chibi-skadi` 是 1536 × 1872 的旧资源且缺少 v2 标记，不能直接作为公开默认宠物；
  已转换出 57 帧、238604 字节、逐帧哈希固定的私人本地包，0.4.2 构建时只覆盖临时工程；
