...
- [ ] 将默认宠物作为 HAP 内置资源，离线时无需网络也能显示和互动；
- [ ] 用真实宠物替换现有 CSS 占位角色，并保持状态、快捷回复、记忆入口和离线提示可用；
- [ ] 轻点、长按或滑动等交互必须有防连点、可中断和低干扰反馈；
- [ ] 动画在应用隐藏、息屏或页面销毁时停止，不维持隐形计时器。
...
- [ ] 服务端提供经过审核的宠物目录，不让手表直接抓取 `codex-pets.net`；
- [ ] 导入器仅从允许的 HTTPS 来源读取有限大小的清单和 PNG/WebP；
- [ ] 校验 MIME、魔数、像素尺寸、图集网格、透明通道、实际帧、解码后大小和重定向次数；
- [ ] 拒绝 SVG、HTML、脚本、路径穿越、外部引用、超大压缩包和未知许可证；
- [ ] 保存来源 URL、作者、许可证、归属说明与源文件 SHA-256；
- [ ] 由服务端或构建流程转换成手表专用资源，手表不解析原始 Codex Pet 包；
- [ ] 目录清单和每个资源文件均提供版本、长度与 SHA-256；
- [ ] 表端先下载到临时版本，全部校验成功后原子切换；失败时继续使用上一个完整版本；
...
- 原创默认宠物 Sprout（芽芽）已完成 hatch-pet v2 图集、三人方向盲测、最终视觉 QA、授权说明与
  手表资源转换；73 帧手表包为 329310 字节，尚未接入表端渲染器；
- 当前表端仍只有 CSS 占位角色，没有逐帧渲染器、宠物清单或动态文件缓存；
