...
2. 手工签名 HAP，并通过已验证的一加 13T 上的华为运动健康和应用调测助手安装到手表；
...
- `assets/pets/watchbuddy-sprout/watch-lite/`：73 帧、96 × 104、约 232 KiB 的 256 色透明 PNG
...
组件是否完整；同时校验 Lite Wearable FA 模型、已验证的 API 17 安装基线、
页面文件、离线权限基线及独立运行源码约束。组件不完整时，按输出提示在
...
已在 AGC 配置调试证书、Profile 和目标设备后，生成可安装的手工签名 HAP：

```bash
npm run build:watch:signed
```
...
服务端文字回复已经接入 OpenAI 与 DeepSeek 两种 AI 适配器。默认保持 OpenAI
`gpt-5.6-terra`；设置 `WATCHBUDDY_AI_PROVIDER=deepseek` 并通过服务器秘密环境变量注入
`DEEPSEEK_API_KEY` 后，使用 `deepseek-v4-flash` 非思考模式。两种适配器均使用 8 秒默认超时，
设备 ID 只以 SHA-256 后的匿名标识发送，模型输出限制为 38 个字符。未配置密钥、超时、截断或
...
也不能自动签名；安装时需由已配对手机上的华为运动健康和应用调测助手选择手工签名 HAP。
已验证的一加 13T 曾通过应用调测助手识别 GT 6 Pro 并成功安装最小探针，证明兼容 Android 手机
可承担开发安装桥接；它仍不会把手表转成 HDC 目标，也不是 WatchBuddy 运行时配套应用。
任何账号凭据、服务端密钥、签名文件与设备令牌均不得提交到 Git。

真机联网前，除将公网 HTTPS 地址写入
`apps/watch-huawei/entry/src/main/js/MainAbility/common/api-config.js` 外，还必须在独立构建中恢复
并验证合法的网络权限配置。仓库默认地址留空，当前 0.1.1 诊断包也不声明网络权限，表端会显示
“待配置服务”，避免误把本地 HTTP、占位地址或离线包当成独立联网结果。
...
Node.js 契约测试与 Lite Wearable 签名构建，仍需配置真实 HTTPS 地址并在 GT 6 Pro 上验证。
