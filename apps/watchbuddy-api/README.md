...
| `GET /v1/pets/:petId/assets/:assetId` | 下载白名单内的单个 PNG/WebP 资源 |

宠物列表、渲染清单、单页资源摘要和每个二进制帧均受 7KB 响应上限约束。资源接口返回
...
当前目录只包含仓库内的 Sprout（芽芽）。动态写入手表文件、原子切换、缓存与回滚尚未完成
Lite Wearable 真机验证，因此这些接口目前只构成服务端受控同步边界，不代表在线换宠物已经开放。
