...
test("鉴权设备可读取受控宠物目录、分页摘要和完整性固定资源", async (t) => {
  ...
  const encodedText = await encodedResponse.text();
  const encoded = JSON.parse(encodedText);
  assert.equal(encodedResponse.status, 200);
  assert.equal(Buffer.byteLength(encodedText) < 7 * 1024, true);
  assert.equal(encoded.asset.id, descriptor.id);
  assert.equal(encoded.asset.encoding, "base64");
  assert.equal(encoded.asset.mediaType, "image/png");
  assert.equal(encoded.asset.bytes, descriptor.bytes);
  assert.equal(
...
test("宠物目录拒绝未鉴权、非法分页、路径穿越和未知资源", async (t) => {
  ...
  const invalidEncoding = await fetch(
    `${baseUrl}/v1/pets/watchbuddy-sprout/assets/idle-0?encoding=hex`,
    { headers: { authorization } }
  );
  assert.equal(invalidEncoding.status, 400);
  assert.equal((await invalidEncoding.json()).error, "invalid_query");
